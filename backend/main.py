...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    shutil.copy(clip_segment_path, pathlib.Path(base_dir) / f"{clip_name}.mp4")
    ...
    s3_client.upload_file(subtitle_output_path, "ai-podcast-clipper-bucket1", output_s3_key)
...
class AiPodcastClipper:
    ...
    def transcribe_video(self, base_dir: str, video_path: str) -> str:
        audio_path = pathlib.Path(base_dir) / "audio.wav"
    ...
    def identify_moments(self, transcript: dict):
        response = self.gemini_client.models.generate_content(model="gemini-2.5-flash",
                contents="""
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("ai-podcast-clipper-bucket1", s3_key, str(video_path))

        # 1. Transcription
        transcript_segments_json  = self.transcribe_video(str(base_dir), str(video_path))
        ...
        for index, moment in enumerate(clip_moments[:1]):
            if "start" in moment and "end" in moment:
                ...
                process_clip(str(base_dir), str(video_path), s3_key, moment["start"], moment["end"], index, transcript_segments)
